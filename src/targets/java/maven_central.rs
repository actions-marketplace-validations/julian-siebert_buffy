...
use crate::{
    configs::profiles::java::MavenCentral,
    error::{Error, Result},
    targets::{
        context::Context,
        java::helpers::{env_nonempty, render_pom, resolve_protobuf_version, verify_compile},
    },
};

pub async fn build_java_profile_maven_target(ctx: Context, m: &MavenCentral) -> Result<()> {
...
pub async fn publish_java_profile_maven_target(ctx: Context, m: &MavenCentral) -> Result<()> {
    ...
    // generate a settings.xml with auth for the `central` server id
...
"#,
    ...
    // Clean up settings.xml regardless of build success
