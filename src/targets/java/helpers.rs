...
pub async fn generate_java_code(ctx: &Context) -> Result<()> {
    let src_dir = ctx.target_path.join("src/main/java");
    crate::io::create_dir_all(&src_dir)?;

    ctx.pb.set_message("Generating Java code...");
    let mut cmd = Command::new(protoc()?);
    cmd.arg(format!("--java_out={}", src_dir.display()))
...
pub fn render_pom(
    ...
    tctx.insert("gpg_passphrase", &env_nonempty("GPG_PASSPHRASE").is_some());
